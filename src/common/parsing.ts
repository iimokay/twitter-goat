...
export function parseJsonArrayFromText(text: string): unknown | null {
  ...
  if (jsonBlockMatch) {
    try {
      ...
      logger.error('Failed parsing text:', jsonBlockMatch[1]);
    }
  }
  ...
  if (!jsonData) {
    ...
    if (arrayMatch) {
      try {
        ...
        logger.error('Error parsing JSON:', e);
        logger.error('Failed parsing text:', arrayMatch[0]);
      }
    }
  }
...
export function parseJSONObjectFromText(text: string): Record<string, unknown> | null {
  ...
  if (jsonBlockMatch) {
    ...
    try {
      ...
      logger.error('Error parsing JSON:', e);
      logger.error('Text is not JSON', text);
...
export function extractAttributes(
  ...
  if (!attributesToExtract || attributesToExtract.length === 0) {
    ...
    attributesToExtract.forEach((attribute) => {
      const match = new RegExp(`"${attribute}"\\s*:\\s*"([^"]*)"?`, 'i').exec(response);
      if (match) {
...
export const normalizeJsonString = (str: string): string => {
